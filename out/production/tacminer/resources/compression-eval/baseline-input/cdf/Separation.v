...
Proof.
intros h2 h1 H; [destruct h1 as [ c1 fin1 ] , h2 as [ c2 fin2 ]; [assert ( c1 = c2 ) by ( apply functional_extensionality ; auto ); [subst c2; [f_equal; [apply proof_irrelevance]]]]].
Qed.
...
Proof.
intros l v h; [cbn; [destruct ( Z.eq_dec l l) as [e | n ]; [congruence |congruence]]].
Qed.

Lemma hupdate_other: forall l v h l', l <> l' -> (hupdate l v h) l' = h l'.
Proof.
intros l v h l' H; [cbn; [destruct ( Z.eq_dec l l' ) as [e | n ]; [congruence |congruence]]].
Qed.
...
Next Obligation.
destruct ( isfinite h ) as ( i & fin ); [exists i; [intros j H; [destruct ( Z.eq_dec l j ) as [e | n ]; [auto |auto]]]].
Qed.
...
Proof.
induction sz;
  [intros l' l H; [auto] |intros l l' H; [cbn; [destruct ( Z.eq_dec l l' ) as [e | n ]; [lia |apply IHsz; [lia]]]]].
Qed.
...
Proof.
unfold hdisjoint; [intros h2 h1; [split; [intros H l; [specialize ( H l ); [tauto]] |intros H l; [specialize ( H l ); [tauto]]]]].
Qed.
...
Proof.
intros h; [apply heap_extensionality; [intros l; [cbn; [auto]]]].
Qed.
...
Proof.
unfold hdisjoint , hunion;
  [cbn;
    [intros h1 h2 h3;
      [split;
        [intros D;
          [split;
            [intros l; [destruct ( D l ) as [H | H ];
              [destruct ( h1 l );
                [auto |auto]
              | auto]]
            |intros l; [destruct ( D l ) as [H | H ];
              ...
              |auto ]]]]
       |intros [D1 D2] l;
         [destruct ( D2 l ) as [H | H ];
           [destruct ( h1 l ) eqn : H1;
              [auto;
                [destruct ( D1 l ) as [H0 | H0 ];
                  [auto; [congruence]
                  |auto]]
              |auto]
            |auto]]]]]].
...
Proof.
intros h1 h2 h H H0 H1;
[apply heap_extensionality;
  [intros l;
    [assert ( hunion h h1 l = hunion h h2 l) by congruence;
    [cbn in H2; [specialize ( H0 l ); [specialize ( H1 l ); [destruct ( h l ); [intuition congruence |intuition congruence ]]]]]]]].
Qed.
...
Proof.
intros h1 h2 h H H0 H1; [apply hunion_invert_r with h; [rewrite <- ! ( hunion_comm h ) by HDISJ; [auto] |HDISJ |HDISJ]].
Qed.
...
Proof.
intros Q P H; [apply functional_extensionality; [intros h; [apply propositional_extensionality; [auto]]]].
Qed.
...
Proof.
  ...
  [exists h1, (hunion h2 h3); [intuition;
  ...
  [exists (hunion h1 h2), h3; [intuition;
...
Proof.
  ...
  | intros; [exists hempty, h; [intuition;
  [ red; [auto]
  | red; [auto]
  | rewrite hunion_empty; [auto]]]]]]]].
Qed.
...
Lemma sepconj_imp_l: forall P Q R,
  (P -->> Q) -> (P ** R -->> Q ** R).
Proof.
intros P Q R IMP h (h1 & h2 & P1 & Q2 & D & U); [exists h1 , h2; [intuition auto]].
Qed.

Lemma sepconj_imp_r: forall P Q R,
  (P -->> Q) -> (R ** P -->> R ** Q).
Proof.
intros P Q R H; [rewrite ! ( sepconj_comm R ); [apply sepconj_imp_l; [auto]]].
Qed.
...
Proof.
  intros P Q; [apply assertion_extensionality; [intros h;
  [unfold pure, sepconj; [split;
    [intros ((A & B) & C); [subst h; [exists hempty, hempty; [intuition;
      [intro l; [auto]
      |rewrite hunion_empty; [auto]]]]]
...
Lemma pureconj_sepconj: forall P Q,
  pure P ** Q = P //\\ Q.
Proof.
intros P Q; [apply assertion_extensionality; [intros h; [unfold pure , sepconj , pureconj; [split; [intros (h1 & h2 & (A & B) & C & D & E); [split; [auto |subst; [rewrite hunion_empty; [auto]]]] |intros (A & B); [exists hempty , h; [intuition; [intro l; [auto] |rewrite hunion_empty; [auto]]]]]]]]].
Qed.

Lemma lift_pureconj: forall P Q R, (P //\\ Q) ** R = P //\\ (Q ** R).
Proof.
intros P Q R; [rewrite <- ! pureconj_sepconj; [rewrite sepconj_assoc; [auto]]].
Qed.

Lemma lift_aexists: forall (A: Type) (P: A -> assertion) Q,
  aexists P ** Q = aexists (fun x => P x ** Q).
Proof.
intros A P Q; [apply assertion_extensionality; [intros h; [split; [intros (h1 & h2 & (a & P1) & Q2 & DISJ & EQ); [exists a , h1 , h2; [auto]] |intros (a & h1 & h2 & P1 & Q2 & DISJ & EQ); [exists h1 , h2; [intuition; [exists a; [auto]]]]]]]].
Qed.
...
Proof.
intros R P Q S; [rewrite ( sepconj_pick2 R ) , ( sepconj_pick2 P ); [auto]].
Qed.
...
Lemma wand_intro: forall P Q R,
  P ** Q -->> R  ->  P -->> Q --* R.
Proof.
intros P Q R IMP h Ph h' DISJ Qh'; [apply IMP; [exists h , h'; [auto]]].
Qed.
...
Proof.
  ...
  [rewrite hunion_comm by auto; apply Wh2; [auto|auto]]]].
Qed.
...
Proof.
  intros P Q; [apply assertion_extensionality; [intros h; [split;
    [intros W; [exists (P --* Q); [split;
      [auto; [apply wand_cancel]
      |auto]]]
    | intros (R & A & B) h' D Ph';
...
Lemma wand_equiv: forall P Q R,
  (P -->> (Q --* R)) <-> (P ** Q -->> R).
Proof.
intros P Q R; [split; [intros H; [intros h (h1 & h2 & Ph1 & Wh2 & D & U); [subst h; [apply H; [auto |auto |auto]]]] |intros H; [apply wand_intro; [auto]]]].
Qed.

Lemma wand_imp_l: forall P P' Q,
  (P' -->> P) -> (P --* Q -->> P' --* Q).
Proof.
intros P P' Q H; [intros h W h' DISJ P'h'; [apply W; [auto |auto]]].
Qed.

Lemma wand_imp_r: forall P Q Q',
  (Q -->> Q') -> (P --* Q -->> P --* Q').
Proof.
intros P Q Q' H; [intros h W h' DISJ Ph'; [apply H; [apply W; [auto |auto]]]].
Qed.

Lemma wand_idem: forall P,
  emp -->> P --* P.
Proof.
intros P h E; [rewrite E; [red; [intros h' H H0; [rewrite hunion_empty; [auto]]]]].
Qed.

Lemma wand_pure_l: forall (P: Prop) Q,
  P -> (pure P --* Q) = Q.
Proof.
intros P Q PP; [apply assertion_extensionality; [intros h; [split; [intros W; [rewrite <- hunion_empty , hunion_comm by HDISJ; [apply W; [HDISJ |split; [auto |auto]]]] |intros Qh h' DISJ (Ph' & E); [subst h'; [rewrite hunion_comm , hunion_empty by HDISJ; [auto]]]]]]].
Qed.

Lemma wand_curry: forall P Q R,
  (P ** Q --* R) = (P --* Q --* R).
Proof.
intros R Q P; [apply assertion_extensionality; [intros h; [split; [intros W h1 D1 Ph1 h2 D2 Qh2; [rewrite hunion_assoc; [apply W; [HDISJ |exists h1 , h2; [intuition auto; [HDISJ]]]]] |intros W h' D (h1 & h2 & Ph1 & Qh2 & D12 & U12); [subst h'; [rewrite <- hunion_assoc; [apply W; [HDISJ |auto |HDISJ |auto]]]]]]]].
Qed.

Lemma wand_star: forall P Q R,
  ((P --* Q) ** R ) -->> (P --* (Q ** R)).
Proof.
intros P Q R; [intros h (h1 & h2 & W1 & R2 & D & U); [subst h; [intros h' D' Ph'; [exists ( hunion h1 h' ) , h2; [intuition; [apply W1; [HDISJ |auto] |HDISJ |rewrite ! hunion_assoc; [f_equal; [apply hunion_comm; [HDISJ]]]]]]]]].
Qed.
...
Proof.
intros X P H x; [red; [intros h1 h2 h1' h2' H0 H1 H2 H3 H4;
[edestruct (H x x h1 h2 h1' h2');
...
Proof.
intros P H; [red; [intros x1 x2 h1 h2 h1' h2' H0 H1 H2 H3 H4; [split;
  [destruct x1, x2; [auto]
  |eauto]]]].
Qed.
...
Lemma pure_precise: forall P,
  precise (pure P).
Proof.
unfold pure; [intros P; [red; [intros h1 h2 h1' h2' H H0 H1 H2 H3; [destruct H2 , H3; [congruence]]]]].
Qed.
...
Proof.
unfold pure; [intros X P H; [red; [intros x1 x2 h1 h2 h1' h2' H0 H1 H2 H3 H4; [destruct H3 , H4; [split; [auto |congruence]]]]]].
Qed.
...
Proof.
intros l v; [apply param_precise_precise; [apply contains_param_precise]].
Qed.

Lemma aexists_precise: forall (X: Type) (P: X -> assertion),
  param_precise P -> precise (aexists P).
Proof.
intros X P H; [red; [intros h1 h2 h1' h2' H0 H1 H2 H3 H4; [destruct H3 as ( x1 & P1 ) , H4 as ( x2 & P2 ); [eapply H; [eauto |eauto | eauto|eauto | eauto]]]]].
Qed.

Lemma valid_precise: forall l,
  precise (valid l).
Proof.
intros l; [apply aexists_precise; [apply contains_param_precise]].
Qed.
...
Proof.
  intros X P Q H H0; [red; [intros x1 x2 h1 h2 h1' h2' H1 H2 H3 H4 H5;
...
[assert (param_precise (fun _ : unit => P ** Q));
  [apply sepconj_param_precise;
    [apply precise_param_precise; [auto]
    |auto]
...
Proof.
intros P1 P2 Q h (h1 & h2 & (P11 & P21) & Q2 & D & E); [split; [exists h1 , h2; [auto] |exists h1 , h2; [auto]]].
Qed.
...
Proof.
intros P2 P1 Q PQ; [rewrite ( sepconj_comm P1 ) , ( sepconj_comm P2 );
[intros h ((h1 & h2 & Q1 & P12 & D & E) & (h1' & h2' & Q1' & P22 & D' & E')); [assert ( h1 = h1' ); [apply PQ with h2 h2'; [auto |auto |congruence |auto |auto]]]]].
subst h1'; [assert ( h2 = h2' ); [apply hunion_invert_r with h1; [congruence |auto |auto]]].
subst h2'; [unfold aand; [exists h2 , h1; [intuition; [HDISJ |rewrite hunion_comm by HDISJ; [auto]]]]].
Qed.
...
Lemma sepconj_self: forall P,
  ...
  P ** P -->> P.
Proof.
intros P H; [intros h (h1 & h2 & P1 & P2 & D & E);
[assert (h1 = h2);
  [apply H with h2 h1;
   [auto |HDISJ |apply hunion_comm; [HDISJ] | auto | auto]
  |subst h2;
  [assert (h = h1);
    [apply heap_extensionality; [intros l; [rewrite E; [cbn; [destruct (h1 l) as [z | ];
      [auto
      |auto]]]]]
    |congruence]]]]].
Qed.
