(** A library of operators over relations,
    ...
    Proof.
    induction 1; [eauto using star |eauto using star  ].
    Qed.
    ...
    Lemma plus_star:
      ...
      plus a b -> star a b.
    Proof.
    intros b a H; [inversion H; [eauto using star]  ].
    Qed.

    Lemma plus_star_trans:
      forall a b c, plus a b -> star b c -> plus a c.
    Proof.
    intros c b a H H0; [inversion H; [eauto using plus , star_trans]  ].
    Qed.
    ...
    Proof.
    induction 1; [econstructor | econstructor; [eauto | eauto  ]  ].
    Qed.
    ...
    | destruct IHstar as ( n & Sn ); [exists ( S n ); [econstructor; [eauto | eauto  ]  ]] ].
    Qed.
    ...
    *)
    ...
    Proof.
    intros a H; [exists ( fun b => b = a ); [split; [auto |intros a1 H0; [subst a1; [exists a; [auto  ] ] ] ] ]].
    Qed.
    ...
    [exists all_seq_inf;
      [split;
        [auto
        |intros a1 ALL1;
          [destruct ( ALL1 a1 ) as [ a2 R12 ];
            [constructor
            | exists a2;
               [split;
                  [auto
                  |intros a3 S23;
                    [destruct ( ALL1 a3 ) as [ a4 R23 ];
    ...
    Proof.
    intros a0 (f & F0 & Fn); [exists ( fun a => exists i , f i = a ); [split; [exists 0; [auto] |intros a1 (i1 & F1); [subst a1; [exists ( f ( 1 + i1 ) ); [split; [auto |exists ( 1 + i1 ); [auto]]]]]]]].
    Qed.
    ...
    Proof.
    intros a (X & Xa & XP); [destruct ( XP a Xa ) as ( b & Rab & Xb ); [exists b; [split; [auto |exists X; [auto]]]]].
    Qed.
    ...
    Proof.
    intros X H a0 Xa0;
    [exists ( fun a => exists b , star a b /\ X b );
      [split;
        [exists a0;
        ...
        |intros a1 (a2 & S12 & X2);
          [inversion S12;
            [subst;
              [destruct ( H a2 X2 ) as ( a3 & P23 & X3 );
                  [inversion P23;
                    [subst;
                      [exists b;
                        [split; [auto |exists a3; [auto]]]]]]]
            | exists b;
                [split;
                  [auto
                  |exists a2; [auto]]]]]]]].
    ...
    Lemma finseq_unique:
      ...
      b = b'.
    Proof.
    intros b' b a H H0 H1 H2;
    [destruct (star_star_inv H H1) as [H3 | H3];
      [inversion H3;
        [auto
        | elim ( H0 _ H4 )]
      |inversion H3;
    ...
    Proof.
    intros a b Rab Ia; [destruct ( infseq_inv Ia ) as ( b' & Rab' & Xb' ); [assert ( b' = b ) by ( eapply R_functional ; eauto ); [subst b'; [auto]]]].
    Qed.

    Lemma infseq_star_inv:
      forall a b, star a b -> infseq a -> infseq b.
    Proof.
    induction 1; [intros H; [auto] |intros H1; [apply IHstar; [apply infseq_inv' with a; [auto |auto]]]].
    Qed.
    ...
    Proof.
    intros a b H H0 H1;
      [destruct ( @infseq_inv b ) as (c & Rbc & _);
    ...
    [unfold all_seq_inf;
      [intros b H0;
        [destruct ( @infseq_inv b ) as ( c & Rbc & _ );
          [eapply infseq_star_inv; [eauto | eauto]
          |exists c; [auto]]]]].
    Qed.
