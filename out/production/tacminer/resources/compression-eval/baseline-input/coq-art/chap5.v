...
Proof.
intros R Q P H H0 p; [apply H0; [apply H; [assumption]]].
Qed.
...
Lemma all_imp_dist   :
 forall (A:Type)(P Q:A->Prop),
         ...
         (forall y:A, P y)->
          forall z:A, Q z.
Proof.
intros A Q P H H0 z; [apply H; [apply H0]].
Qed.


Lemma mult_le_compat_r : forall m n p:nat, le n p -> le (n * m) (p * m).
Proof.
intros m n p H; [rewrite ( Nat.mul_comm n m ); [rewrite ( Nat.mul_comm p m ); [apply Nat.mul_le_mono_l; [assumption]]]].
Qed.
...
Proof.
intros a b c d H0 H; [apply Nat.le_trans with ( m := c * b ); [apply mult_le_compat_r; [assumption] |apply Nat.mul_le_mono_l; [assumption]]].
Qed.
...
Lemma le_mult_mult' :
 forall a b c d:nat, a <= c -> b <= d -> a*b <= c*d.
Proof.
intros d c b a H H0; [eapply Nat.le_trans; [eapply Nat.mul_le_mono_l; [eexact H0] |apply mult_le_compat_r; [assumption]]].
Qed.

Lemma le_O_mult : forall n p:nat, 0 * n <= 0 * p.
Proof.
intros p n; [apply le_n].
Qed.
...
Proof.
intros p n H; [unfold lt; [apply le_S; [assumption]]].
Qed.
...
Proof.
intros y x Heq; [unfold Zsquare_diff at 1; [rewrite Heq; [ring]]].
Qed.
...
Proof.
intros Q P p H; [elim H; [assumption]].
Qed.

Theorem double_neg_i : forall P:Prop, P->~~P.
Proof.
intros P p H; [apply H; [assumption]].
Qed.
...
Proof.
intros B A; [unfold not; [apply imp_trans]].
Qed.
...
Proof.
intros B A H; [destruct H as [ H0 H ]; [split; [assumption |assumption]]].
Qed.

Lemma or_commutes : forall A B:Prop, A\/B->B\/A.
Proof.
intros B A H; [destruct H as [ H | H ]; [auto | auto]].
Qed.

Lemma ex_imp_ex :
 forall (A:Type)(P Q:A->Prop), (ex P)->(forall x:A, P x -> Q x)->(ex Q).
Proof.
intros A Q P H H0; [destruct H as [ a Ha ]; [exists a; [apply H0; [assumption]]]].
Qed.
...
Proof.
intros A b a e; [rewrite e; [reflexivity]].
Qed.

Lemma Zmult_distr_1 : forall n x:Z, n * x + x = ( n + 1) * x.
Proof.
intros x n; [rewrite Zmult_plus_distr_l; [now rewrite Zmult_1_l]].
Qed.
...
Lemma conditional_rewrite_example : forall n:nat,
   8 <= n + 6 ->  3 + n < 6 -> n * n = n + n.
Proof.
intros n H H0; [rewrite <- ( le_lt_S_eq 2 n ); [reflexivity |apply Nat.add_le_mono_l with ( p := 6 ); [auto with arith] |apply Nat.add_lt_mono_l with ( p := 3 ); [auto with arith]]]. rewrite Nat.add_comm in H.
Qed.
...
Proof.
intros A z y x H; [rewrite H; [auto]].
Qed.
...
Proof.
intros y x H Q; [apply H; [trivial]].
Qed.
