...
Proof.
induction 1; [auto with sorted_base arith |auto with sorted_base arith |auto with sorted_base arith].
Qed.

Theorem sorted1_inv {A:Type}{le : relation A} { x l} (H: sorted le (x::l))  :
  sorted le l.
Proof.
inversion H; [auto with sorted_base |auto with sorted_base].
Qed.
...
Proof.
intros n2 n1 t1 t2 H; [injection H; [intros H1 H2; [dependent rewrite <- H1; [trivial]]]].
Qed.
...
Section vectors_and_lists.
 ...
 Proof.
intros n v; [induction v; [simpl; [auto] |simpl; [auto]]].
Qed.
 ...
 Proof.
intros a m n Heq; [rewrite Heq; [intros w v HJeq; [rewrite HJeq; [reflexivity]]]].
Qed.
 ...
 Proof.
intros n v; [induction v; [reflexivity |simpl; [apply Vconseq; [now rewrite keep_length |assumption]]]].
Qed.
...
Proof.
intros  A B [Ha | [Hb Hi]]; [assumption |now apply Hi].
Qed.

Theorem sum_even : forall n p:nat, even n -> even p -> even (n+p).
Proof.
intros p n Heven_n; [induction Heven_n; [trivial |intro H0; [simpl; [constructor; [auto]]]]].
Qed.
...
Proof.
intros p n H; [induction H; [repeat constructor |repeat constructor; [assumption]]].
Qed.
...
Proof.
apply Pfact1 with ( n := 3 ) ( v := 2 ); [discriminate |apply ( Pfact1 2 1 ); [discriminate |apply ( Pfact1 1 1 ); [discriminate |apply Pfact0]]].
Qed.

Theorem fact_def_pos : forall x y:Z, Pfact x y ->  0 <= x.
Proof.
intros y x H; [induction H; [auto with zarith |lia]].
Qed.
...
[intros Hle;
  [destruct  (Zle_lt_or_eq  _ _ Hle);
    [destruct (H (x-1));
      [unfold Zwf; [lia]
      | lia
      | exists (x*x0); [apply Pfact1; [auto with zarith | auto with zarith]]]
...
Proof.
induction 1; [simpl; [auto] |simpl; [auto]].
Qed.
...
Proof.
intro H; [generalize ( refl_equal 1 ); [pattern 1 at - 2; [induction H; [discriminate |discriminate]]]].
Qed.

Theorem plus_2_even_inv' : forall n:nat, even (S (S n))-> even n.
Proof.
intros n H; [generalize ( refl_equal ( S ( S n ) ) ); [pattern ( S ( S n ) ) at - 2; [induction H; [discriminate |intros H0; [injection H0; [intro H1; [now subst n0]]]]]]].
Qed.