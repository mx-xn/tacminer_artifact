...
Lemma hupdate_dup (l: addr) (v: Z) (h: heap) : exists i : Z,
forall j : Z, i <= j -> (if Z.eq_dec l j then Some v else h j) = None.
Proof.
destruct (isfinite h) as (i & fin).
exists (Z.max i (l + 1)); intros.
destruct (Z.eq_dec l j). lia. apply fin; lia.
...
Proof.
  intros l v h. cbn. destruct (Z.eq_dec l l); congruence.
Qed.

Lemma hupdate_other: forall l v h l', l <> l' -> (hupdate l v h) l' = h l'.
Proof.
  intros l v h l' H. cbn. destruct (Z.eq_dec l l'); congruence.
Qed.
...
Lemma hfree_dup (l: addr) (h: heap) : exists i : Z,
forall j : Z, i <= j -> (if Z.eq_dec l j then None else h j) = None.
Proof.
destruct (isfinite h) as (i & fin).
exists i; intros. destruct (Z.eq_dec l j); auto.
Qed.
...
Proof.
  intros h sz. induction sz; intros.
- lia.
- cbn. destruct (Z.eq_dec l l'); auto. apply IHsz. lia.
Qed.

Lemma hinit_outside:
  forall h sz l l', l' < l \/ l + Z.of_nat sz <= l' -> hinit l sz h l' = h l'.
Proof.
  intros h sz. induction sz; intros; cbn.
- auto.
...
Proof.
  unfold hdisjoint; intros h1 h2; split; intros H l; specialize (H l); tauto.
Qed.
...
Lemma hunion_dup (h1 h2 : heap) : exists i : Z,
forall j : Z, i <= j -> (if h1 j then h1 j else h2 j) = None.
Proof.
destruct (isfinite h1) as (i1 & fin1), (isfinite h2) as (i2 & fin2).
exists (Z.max i1 i2); intros. rewrite fin1, fin2 by lia. auto.
Qed.
...
Proof.
  intros h1 h2 H; apply heap_extensionality; intros l; cbn.
  specialize (H l). destruct (h1 l) as [z|], (h2 l) as [z'|]; intuition congruence.
Qed.

Lemma hunion_assoc:
  forall h1 h2 h3, hunion (hunion h1 h2) h3 = hunion h1 (hunion h2 h3).
Proof.
  intros h1 h2 h3; apply heap_extensionality; intros l; cbn. destruct (h1 l) as [z|]; auto.
Qed.
...
Proof.
  unfold hdisjoint, hunion; cbn; intros h1 h2 h3. split.
- intros D; split; intros l; destruct (D l) as [z|]; auto; destruct (h1 l) as [z'|]; auto.
  discriminate.
- intros [D1 D2] l. destruct (D2 l) as [z|]; auto. destruct (h1 l) as [z'|] eqn:H1; auto. destruct (D1 l) as [z''|]; auto. congruence.
Qed.
...
Proof.
  ...
  cbn in H2. specialize (H0 l); specialize (H1 l). destruct (h l) as [z|]; intuition congruence.
Qed.
...
Proof.
  ...
  intros P Q; apply assertion_extensionality; intros h. split; auto.
Qed.

Lemma sepconj_assoc: forall P Q R, (P ** Q) ** R = P ** (Q ** R).
Proof.
  intros P Q R; apply assertion_extensionality; intros h; split.
...
Proof.
  intros P; apply assertion_extensionality; intros h. split.
...
- intros (h1 & h2 & (PP & E1) & (QQ & E2) & C & D).
  subst h h1 h2. rewrite hunion_empty; auto.
Qed.
...
- intros (h1 & h2 & (A & B) & C & D & E).
  split. auto. subst h h1. rewrite hunion_empty. auto.
- intros (A & B). exists hempty, h; intuition.
  intro l. auto.
  rewrite hunion_empty; auto.
Qed.
...
Proof.
  intros A P Q; apply assertion_extensionality; intros h; split.
...
Proof.
  intros X P H x; red; intros h1 h2 h1' h2' H0 H1 H2 H3 H4. edestruct (H x x h1 h2 h1' h2'); eauto.
Qed.

Remark precise_param_precise:
  forall P, precise P -> param_precise (fun _ : unit => P).
Proof.
  intros P H; red; intros x1 x2 h1 h2 h1' h2' H0 H1 H2 H3 H4. split. destruct x1, x2; auto. eauto.
Qed.
...
Proof.
  unfold contains; intros l; red; intros x1 x2 h1 h2 h1' h2' H H0 H1 H2 H3.
...
Proof.
  intros X P H; red; intros h1 h2 h1' h2' H0 H1 H2 H3 H4. destruct H3 as (x1 & P1), H4 as (x2 & P2).
  eapply H; eauto.
Qed.
...
Proof.
  ...
  subst h2.
  assert (h = h1). apply heap_extensionality; intros l. rewrite E; cbn. destruct (h1 l) as [z|]; auto.
  congruence.
Qed.