...
Lemma star_trans : forall ( a b : A ), star a b -> forall c, star b c -> star a c .
Proof.
   induction 1.
    - eauto using star.
    - eauto using star.
...
Lemma plus_star : forall a b, plus a b -> star a b .
Proof.
   intros a b H. inversion H. eauto using star.
Qed.

Lemma plus_star_trans : forall a b c, plus a b -> star b c -> plus a c .
Proof.
   intros a b c H H0. inversion H. eauto using plus, star_trans.
Qed.

Lemma star_plus_trans : forall a b c, star a b -> plus b c -> plus a c .
Proof.
   intros a b c H H0. inversion H0. inversion H.
    - eauto using plus, star, star_trans.
    - eauto using plus, star, star_trans.
Qed.
...
Lemma starN_star : forall n a b, starN n a b -> star a b .
Proof.
   induction 1.
    - econstructor.
    - custom4.
...
Lemma star_starN : forall a b, star a b -> exists n, starN n a b .
Proof.
   induction 1.
    - exists O. constructor.
    - destruct IHstar as ( n & Sn ). exists ( S n ). custom4.
Qed.
...
Proof.
   intros a H. exists ( fun b => b = a ). custom2 a1 H0. subst a1. custom7 a.
Qed.
...
Proof.
   intros a0 ALL0. exists all_seq_inf. custom2 a1 ALL1. destruct ( ALL1 a1 ) as [ a2 R12 ].
    - constructor.
    - exists a2. custom2 a3 S23. destruct ( ALL1 a3 ) as [ a4 R23 ].
      -- custom5 star_step a2.
      -- custom7 a4.
Qed.
...
Proof.
   intros a0 ( f & F0 & Fn ). exists ( fun a => exists i, f i = a ). split.
    - exists 0. auto.
    - intros a1 ( i1 & F1 ). subst a1. exists ( f ( 1 + i1 ) ). split.
      -- auto.
      -- exists ( 1 + i1 ). auto.
Qed.
...
Proof.
   intros a ( X & Xa & XP ). destruct ( XP a Xa ) as ( b & Rab & Xb ). custom0 b X.
Qed.
...
Lemma infseq_coinduction_principle : forall ( X : A -> Prop ), ( forall a, X a -> exists b, plus a b /\ X b ) -> forall a, X a -> infseq a .
Proof.
   intros X H a0 Xa0. exists ( fun a => exists b, star a b /\ X b ). split.
    - exists a0. auto using star_refl.
    - intros a1 ( a2 & S12 & X2 ). inversion S12.
      -- destruct ( H a2 X2 ) as ( a3 & P23 & X3 ). inversion P23. custom0 b a3.
      -- custom0 b a2.
Qed.
...
Proof.
   induction 1.
    - intros c H. auto.
    - intros c0 H1. inversion H1.
      -- subst a0 a. right. eauto using star.
      -- apply IHstar. custom3 b b0 R_functional.
Qed.
...
- inversion H3. subst a0. auto. elim (H0 _ H4).
- inversion H3. subst a0. auto. elim (H2 _ H4).
Qed.
...
Proof.
   intros a b Rab Ia. destruct ( infseq_inv Ia ) as ( b' & Rab' & Xb' ). custom3 b' b R_functional.
Qed.
...
Proof.
   induction 1.
    - intros H. auto.
    - intros H1. apply IHstar. custom5 infseq_inv' a.
Qed.
...
Proof.
   intros a b H H0 H1. custom1 b infseq_star_inv. apply ( H0 c ). auto.
Qed.
...
Proof.
   intros a H. unfold all_seq_inf. intros b H0. custom1 b infseq_star_inv. custom7 c.
Qed.
