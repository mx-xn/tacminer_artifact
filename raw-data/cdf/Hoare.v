...
Proof.
  intros R.
  assert (IND_SUB: forall (vars1 vars2: ident -> Prop), independent R vars1 -> (forall x, vars2 x -> vars1 x) -> independent R vars2).
...
Proof.
  ...
  assert (REC: forall v s, P s -> aeval variant s = v -> safe (afalse b //\\ P) (WHILE b c) s ).
    intro v. induction v using (well_founded_induction (Z.lt_wf 0)).
...
Proof.
  intros P b c T.
  assert (REC: forall s, P s -> safe (afalse b //\\ P) (WHILE b c) s).
...
Proof.
  intros Q R c2 n. induction n; intros QR c s SF.
- constructor.
...
Lemma triple_while: forall P b c,
   ...
   ⦃⦃ P ⦄⦄ WHILE b c ⦃⦃ afalse b //\\ P ⦄⦄.
Proof.
  intros P b c T. red. intro n. induction n; intros s Ps. constructor.
  apply safe_step. unfold terminated; congruence. cbn; auto.
  intros c' s' RED; inv RED.
