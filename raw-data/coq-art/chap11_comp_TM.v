...
(* Ltac custom9 H0 H1 H2 H3 H4 H6 H5 :=  intros H0 H1 H2 H3; [elim H2; [intros H4; [absurd ( H0 < H6 ); [apply H5 | auto | .. ] | .. ] | .. ] | .. ].
Ltac custom0 H0 :=  intro H0; [elim H0 | .. ].
Ltac custom29 H0 H1 H2 :=  absurd ( occ H0 H1 ); [apply H2 | auto | .. ].
Ltac custom45 H0 :=  apply H0; [auto with zarith | .. ].
Ltac custom3 H0 :=  elim H0; [auto with searchtrees | .. ]. 
Ltac custom10 H0 H1 H2 H3 :=  elim ( occ_inv H0 ); [ | custom0 H1; [intro H2 | intro H3 | .. ] | .. ].
Ltac custom16 H0 H1 :=  intros H0 H1; [elim ( occ_inv H1 ) | .. ].
Ltac custom41 H0 H1 H2 :=  apply H0; [ |  | custom16 H1 H2 | .. ]. 
Ltac custom40 H0 H1 :=  intro H0; [elim H1 | .. ]. 
Ltac custom6 H0 :=  apply H0; [auto with searchtrees | auto with searchtrees | auto with searchtrees | auto with searchtrees | .. ]. *)
...
Proof.
  custom9 z t H H' H0 z Z.lt_irrefl. 
Qed.
...
Section search_tree_basic_properties.
  ...
  Proof.
    intros p H H0; elim (occ_inv H).
    - custom0 e. absurd (n < p).
      + rewrite e; custom45 Zle_not_lt.
      + apply Z.gt_lt. assumption. 
    -  destruct 1; auto. 
        custom29 p t1 not_left. 
        unfold Z.ge; rewrite H0; discriminate.
  Qed.
...
Proof.
refine (fix occ_dec (p:Z) (t:Z_btree) {struct t} : occ_dec_spec p t := match t as x return occ_dec_spec p x with | Z_leaf => fun h => right _ _ | Z_bnode n t1 t2 => fun h => match Z_le_gt_dec p n with | left h1 => match Z_le_lt_eq_dec p n h1 with | left h'1 => match occ_dec p t1 _ with | left h''1 => left _ _ | right h''2 => right _ _ end | right h'2 => left _ _ end | right h2 => match occ_dec p t2 _ with | left h''1 => left _ _ | right h''2 => right _ _ end end end); eauto with searchtrees.
...
Proof.
refine (fix list2tree_aux (l:list Z) : forall t:Z_btree, list2tree_aux_spec l t := fun t => match l return list2tree_aux_spec l t with | nil => fun s => exist _ t _ | cons p l' => fun s => match insert p (t:=t) s with | exist _ t' _ => match list2tree_aux l' t' _ with | exist _ t'' _ => exist _ t'' _ end end end).
  -  split; auto.
     +  split; auto.
        *     induction 1; auto.
              inversion H.
...
Definition list2tree : forall l:list Z, list2tree_spec l.
  Proof.
  refine (fun l => match list2tree_aux l (t:=Z_leaf) _ with | exist _ t _ => exist _ t _ end).
...
let list2tree l =
  ...
  Section RMAX_np.
    ...
    Ltac custom3 H0 :=  elim H0; [auto with searchtrees | .. ]. 
    Ltac custom10 H0 H1 H2 H3 :=  elim ( occ_inv H0 ); [ | custom0 H1; [intro H2 | intro H3 | .. ] | .. ].
    Ltac custom26 H0 :=  rewrite H0; [auto with searchtrees | .. ].
    Lemma rmax_7 :
      forall q':Z,
        occ q' (Z_bnode n t1 t') -> occ q' (Z_bnode n t1 (Z_bnode p t2 t3)).
    Proof.
      intros q' H; custom10 H H0 H1 H1. 
      - intro H0. custom26 H0. 
      - custom3 R1. 
      - custom3 R1. 
