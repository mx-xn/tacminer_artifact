...
Proof. intros p0 H; case (occ_inv H). - intro eg; elim R; rewrite eg; auto with searchtrees. - intro D; elim D; auto with searchtrees. intro H'; elim R; auto with searchtrees. Qed.
...
Ltac custom_tac46 H0 := auto; intro H0.

Ltac custom_tac73 H0 := auto; rewrite H0.

Ltac custom_tac16 H0 := auto with searchtrees; apply H0.

Lemma rm_6 : search_tree (Z_bnode q t0 t'). Proof. right. - elim R; auto with searchtrees. - apply search_tree_r with n (Z_bnode p t1 t2); auto with searchtrees. - elim R; intros H H0 H1 H2 H3 H4; apply maj_intro; intros q0 H5. elim (Zle_lt_or_eq _ _ (H0 q0 (H1 q0 H5))); auto. + intro eg; absurd (occ q0 t0);auto. rewrite eg; auto with searchtrees. - apply min_intro; intros q0 H;  apply Z.lt_trans with n. + elim R; auto with searchtrees. + elim (min_r (n:=n) (t1:=(Z_bnode p t1 t2)) (t2:=t'));
