...
class CoqLineByLineReader:
    ...
    # Detects semicolons, but only those on the outermost level
    def splittable_semicolon(tactic: str, signature: bool = False) -> int:
        words = tactic.split(";")

        # if not signature:
        #     for word in words: # Special syntax for branching. Ideally we should support this, but it's quite difficult to manage...
        #         if word.strip()[0] == '[':
        #             return -1

        paren_count = 0
        for idx, c in enumerate(tactic):
            if c in '([':
                paren_count += 1
            elif c in ')]':
                paren_count -= 1
