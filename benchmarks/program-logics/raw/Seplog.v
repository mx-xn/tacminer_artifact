...
- constructor. cbn. destruct (h l) as [z|]; congruence.
- constructor. cbn. destruct (h l) as [z|]; congruence.
- constructor. cbn. destruct (h l) as [z|]; congruence.
- constructor.
Qed.
...
- exists (hinit l sz h1); intuition auto.
  + constructor; auto. intros. apply H in H1. cbn in H1. destruct (h1 i) as [z|]; congruence.
  + red; cbn; intros i.
    assert (EITHER: l <= i < l + Z.of_nat sz \/ (i < l \/ l + Z.of_nat sz <= i)) by lia.
    destruct EITHER.
    * right. apply H in H1. cbn in H1. destruct (h1 i) as [z|], (h2 i) as [z'|]; congruence.
...
- exists h1; intuition auto. constructor; auto. cbn in H. destruct (h1 l) as [z|]; congruence.
...
Proof.
  intro l. induction l as [ | h t]; intros; rewrite (unroll_com (list_length_rec a len)); cbn.
...
Proof.
  intros l2 a2 l1. induction l1 as [ | h1 t1]; intros; rewrite (unroll_com (list_concat_rec a1 a2)); simpl.
...
Proof.
  intro l. induction l as [ | hd l]; intros; rewrite (unroll_com (list_rev_rec a p)); simpl.
...
Proof.
  intros R sz. induction sz as [ | sz]; intros l h Rh EMPTY; cbn.
...
Proof.
  intros; red; intros. apply wp_set.
  eapply sepconj_imp_r.
  - instantiate (1 := contains l v --* Q).
    intros h' H' v'. auto.
  - eauto.
Qed.
...
Proof.
  intros; red; intros. apply wp_free. eapply sepconj_imp_r.
  - instantiate (1 := Q).
    intros h' H' v'. auto.
  - eauto.
Qed.
