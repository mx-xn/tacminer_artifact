...
Proof.
  intros R Q J n. induction n; intros.
- constructor.
- inv H.
  + constructor. exists h, h'; auto.
  + constructor; auto.
  * intros. apply ACC in H. cbn. destruct (h l) as [z|]; congruence.
  * intros. subst h0.
  ...
  * intros. subst h0.
    ...
    eauto.
    eauto.
    rewrite (hunion_comm (hunion h' hf)) by HDISJ. rewrite hunion_assoc.
    rewrite <- hunion_assoc.
    rewrite (hunion_comm hj) by HDISJ.
    eauto.
    exists (hunion h1' h'), hj'.
    split. HDISJ.
    split. rewrite hunion_assoc. f_equal.
    rewrite (hunion_comm (hunion hj' hf)) by HDISJ.
    rewrite hunion_assoc.
    rewrite (hunion_comm h') by HDISJ.
    assumption.
    split. assumption.
...
Lemma safe_frame_invariant:
  ...
  safe n c h Q (J ** J').
Proof.
  intros Q J J' n. induction n; intros.
- constructor.
- inv H.
  + constructor; auto.
  + constructor; auto.
...
Proof.
  intros Q R J f n. induction n; intros until h; intros S1 S2.
- constructor.
- constructor; auto; intros.
...
Proof.
  intros J P c Q TR IMP. red; intro n; induction n; intros h Ph.
- constructor.
- constructor; auto.
  + intros; intro ST. inv ST.
...
Proof.
  intros J Q1 Q2 n. induction n; intros until h2; intros S1 S2 DISJ; constructor; auto.
- cbn; intros. destruct H as [H | H].
    + eapply safe_immacc with (h1 := h1) in H; eauto.
      destruct (h1 l) as [z|]; congruence.
    + eapply safe_immacc with (h1 := h2) in H; eauto.
      destruct (h1 l) as [z'|]; congruence.
- intros. intros ST; inv ST.
  + destruct H3 as [IM1 IM2].
    eapply safe_immacc with (h1 := h1) in IM1; eauto.
    eapply safe_immacc with (h1 := h2) in IM2; eauto.
...
Proof.
  intro sz. induction sz as [ | sz]; intros l; cbn.
- red; auto.
...
Lemma safe_consequence:
  ...
  safe n c h Q J.
Proof.
  intros Q Q' J H n. induction n; intros.
- constructor.
- inv H0.
...
Proof.
  intros J Q Q' H n. induction n; intros c h S S'.
- constructor.
- inv S; inv S'.
...
Proof.
  ...
  eapply triple_let with (Q := fun v' => ((v' = v) //\\ contains lck v)**(if v =? 0 then emp else R)).
...
Proof.
  ...
  assert ((contains lck 1**R) h).
    eapply sepconj_imp_r with (P := (if v =? 0 then emp else R) ** R); eauto.
    destruct (v =? 0).
...
- apply triple_atomic.
  ...
  + apply triple_simple_conj_pre; intros NOTZERO.
    ...
    eapply triple_seq.
     ...
     eapply sepconj_imp_r with (P := contains (p + 1) t ** R x ** contains buff t ** list_invariant R l t); eauto.
