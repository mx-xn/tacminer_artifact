...
Proof.
 apply check_correct; auto with arith.
...
Proof.
 ...
 intros t1 IHt1 t2 IHt2; rewrite <- flatten_aux_valid_A; rewrite <- IHt2. auto. 
Qed.
 ...
 -  intros n0; case_eq n0.
   ...
   + unfold prime_test; intros n1 Heqn0 Heqn.
     ...
     * rewrite <- Heqn.
       ...
       lia.
       assert (Hex: exists k':nat, (1 < (Z_of_nat k') <= (Z.sqrt (Z_of_nat n))) /\ (exists q':nat, n=(k'*q')%nat)).
       ...
       elim (test_odds_correct (Z.abs_nat (Zpos p')) n (Zpos p)) with k'.
       ...
       auto.
       intros p' Hp; rewrite Hp in Htest_eq.
       elim (test_odds_correct (Z.abs_nat (Zpos p')) n (Zpos p + 1)) with k'.
       rewrite Z_to_nat_and_back.
