...
Ltac custom_tac7 H0 H1 := unfold H0; apply H1.
...
Proof. intros P Q R H H0 p. apply H0. custom_tac1 H. Qed.
...
Proof. intros m n p H. rewrite (Nat.mul_comm n m). rewrite (Nat.mul_comm p m). 
custom_tac1 Nat.mul_le_mono_l. Qed. 

Lemma le_mult_mult :
   forall a b c d:nat, a <= c -> b <= d -> a * b <= c * d.
Proof. 
  intros a b c d H H0. apply Nat.le_trans with (m := c * b). 
- custom_tac1 mult_le_compat_r.
- custom_tac1 Nat.mul_le_mono_l.
Qed.
...
Proof. intros n p H. unfold lt; custom_tac1 le_S. Qed.
...
Proof. intros A B; custom_tac7 not imp_trans. Qed.
...
Proof. intros A P Q H H0; destruct H as [a Ha]. exists a; custom_tac1 H0. Qed. 
